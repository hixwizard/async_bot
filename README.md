...
docker compose up --build
```
8) Запустить интерактивную оболочку bash внутри контейнера с именем 'infra-admin-1'
```
docker exec -it infra-admin-1 bash
```
9) Перейти в директорию admin
```
cd/admin
```
10) Создать суперпользователя с именем 'user' и паролем 'password'
```
flask create_superuser user password 
```
11) Открыть админку по ссылке http://127.0.0.1:5000/admin/
12) Открыть бота в тг и запустить его командой "/start"
